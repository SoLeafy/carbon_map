			...
			if(request) {
				...
				if(request.ok) {
					...
					if (json.features.length === 0) {
						...
						// 생성한 Feature로 VectorSource 생성 new VectorSource는 안되더라
						const vector = new ol.source.Vector({ features: [feature] });
						//console.log(feature.getId()); // 굳이
						console.log(feature.get('bjd_nm'));
						console.log(feature.get('totusage'));
						
						overlay.setPosition(ol.extent.getCenter(vector.getExtent())); // <- getCenter()가 없다는데 뭘까
					}
				} else {
